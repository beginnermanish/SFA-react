...
;(function($) {
    ...
    var buttonYes = '<button class="sfa-btn" data-type="%type%"><span class="sfa-icon sfa-icon-popup-ok"></span></button>';
    var buttonNo = '<button class="sfa-btn" data-type="%type%"><span class="sfa-icon sfa-icon-popup-close"></span></button>';
    ...
    $.messageBox = function(element, options) {
        ...
        var createButton = function(type, text){
            var s = '';
            if( type == BTN_NO ){
                s = buttonNo.replace(/%type%/g, type).replace(/%text%/g, plugin.settings.locale.NO);
            }else if( type == BTN_YES ){
                s = buttonYes.replace(/%type%/g, type).replace(/%text%/g, plugin.settings.locale.YES);
            }else if( type == BTN_CANCEL ){
                s = buttonNo.replace(/%type%/g, type).replace(/%text%/g, plugin.settings.locale.CANCEL);
            }else if( type == BTN_OK){
                s = buttonYes.replace(/%type%/g, type).replace(/%text%/g, plugin.settings.locale.OK);
            }else{
